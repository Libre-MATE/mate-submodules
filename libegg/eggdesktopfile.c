 ...
 * Return value: the new #EggDesktopFile, or %NULL on error.
 **/
EggDesktopFile *egg_desktop_file_new_from_data_dirs(
    const char *desktop_file_path, GError **error) {
...
EggDesktopFile *egg_desktop_file_new_from_key_file(GKeyFile *key_file,
  ...
  if (desktop_file->icon && !g_path_is_absolute(desktop_file->icon)) {
    ...
    if (ext && (!strcmp(ext, ".png") || !strcmp(ext, ".xpm") ||
                !strcmp(ext, ".svg"))) {
      g_warning(
          "Desktop file '%s' has malformed Icon key '%s'"
 ...
 **/
EggDesktopFileType egg_desktop_file_get_desktop_file_type(
    EggDesktopFile *desktop_file) {
  ...
  if (desktop_environment) {
    ...
    if (only_show_in != NULL) {
      for (i = 0, found = FALSE; only_show_in[i] && !found; i++) {
        if (strcmp(only_show_in[i], desktop_environment) == 0) found = TRUE;
      }
    ...
    if (not_show_in != NULL) {
      for (i = 0, found = FALSE; not_show_in[i] && !found; i++) {
        if (strcmp(not_show_in[i], desktop_environment) == 0) found = TRUE;
      }
...
static GSList *translate_document_list(EggDesktopFile *desktop_file,
  ...
  for (d = documents, ret = NULL; d; d = d->next) {
    ...
    if (translated) ret = g_slist_prepend(ret, translated);
  }
...
static void free_document_list(GSList *documents) {
  ...
  for (d = documents; d; d = d->next) g_free(d->data);
...
static gboolean egg_desktop_file_launchv(EggDesktopFile *desktop_file,
  ...
  while ((option = va_arg(args, EggDesktopFileLaunchOption))) {
    switch (option) {
      case EGG_DESKTOP_FILE_LAUNCH_CLEARENV:
        if (env) g_ptr_array_free(env, TRUE);
        env = g_ptr_array_new();
        break;
      case EGG_DESKTOP_FILE_LAUNCH_PUTENV:
        variables = va_arg(args, char **);
        for (i = 0; variables[i]; i++) env = array_putenv(env, variables[i]);
        break;
  ...
  do {
    ...
    if (GDK_IS_X11_DISPLAY(display)) {
      ...
      startup_id = start_startup_notification(
          display, desktop_file, argv[0], screen_num, workspace, launch_time);
      if (startup_id) {
        char *startup_id_env =
    ...
    if (env != NULL) g_ptr_array_add(env, NULL);
...
static void egg_set_desktop_file_internal(const char *desktop_file_path,
  ...
  if (egg_desktop_file) egg_desktop_file_free(egg_desktop_file);
  ...
  if (set_defaults && egg_desktop_file != NULL) {
    ...
    if (egg_desktop_file->name) g_set_application_name(egg_desktop_file->name);
