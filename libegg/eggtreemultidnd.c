...
static void egg_tree_multi_drag_drag_data_get(GtkWidget *widget,
...
static gboolean egg_tree_multi_drag_motion_event(GtkWidget *widget,
  ...
  if (gtk_drag_check_threshold(widget, priv_data->x, priv_data->y, event->x,
                               event->y)) {
    ...
    if (egg_tree_multi_drag_source_row_draggable(
      ...
      context = gtk_drag_begin_with_coordinates(
          widget, gtk_drag_source_get_target_list(widget), di->source_actions,
          priv_data->pressed_button, (GdkEvent *)event, event->x, event->y);
...
static gboolean egg_tree_multi_drag_button_press_event(GtkWidget *widget,
  ...
  gtk_tree_view_get_path_at_pos(tree_view, event->x, event->y, &path, &column,
                                &cell_x, &cell_y);
  ...
  if (path && gtk_tree_selection_path_is_selected(selection, path)) {
    ...
    priv_data->motion_notify_handler =
        g_signal_connect(tree_view, "motion_notify_event",
