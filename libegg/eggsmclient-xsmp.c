...
#define EGG_SM_CLIENT_XSMP(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), EGG_TYPE_SM_CLIENT_XSMP, EggSMClientXSMP))
#define EGG_SM_CLIENT_XSMP_CLASS(klass)                      \
  (G_TYPE_CHECK_CLASS_CAST((klass), EGG_TYPE_SM_CLIENT_XSMP, \
                           EggSMClientXSMPClass))
#define EGG_IS_SM_CLIENT_XSMP(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), EGG_TYPE_SM_CLIENT_XSMP))
#define EGG_IS_SM_CLIENT_XSMP_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE((klass), EGG_TYPE_SM_CLIENT_XSMP))
#define EGG_SM_CLIENT_XSMP_GET_CLASS(obj)                    \
  (G_TYPE_INSTANCE_GET_CLASS((obj), EGG_TYPE_SM_CLIENT_XSMP, \
                             EggSMClientXSMPClass))
...
static const char *state_names[] = {"idle",
                                    "save-yourself",
                                    "interact-request",
                                    "interact",
                                    "save-yourself-done",
...
static void sm_client_xsmp_startup(EggSMClient *client, const char *client_id);
static void sm_client_xsmp_set_restart_command(EggSMClient *client, int argc,
                                               const char **argv);
static void sm_client_xsmp_set_discard_command(EggSMClient *client, int argc,
                                               const char **argv);
...
static void egg_sm_client_xsmp_init(EggSMClientXSMP *xsmp) {
  xsmp->state = XSMP_STATE_CONNECTION_CLOSED;
  xsmp->connection = NULL;
  xsmp->restart_style = SmRestartIfRunning;
}

static void egg_sm_client_xsmp_class_init(EggSMClientXSMPClass *klass) {
...
EggSMClient *egg_sm_client_xsmp_new(void) {
  if (!g_getenv("SESSION_MANAGER")) return NULL;

  return g_object_new(EGG_TYPE_SM_CLIENT_XSMP, NULL);
}

static gboolean sm_client_xsmp_set_initial_properties(gpointer user_data) {
 ...
 */
static void sm_client_xsmp_disconnect(EggSMClientXSMP *xsmp) {
  SmcConn connection;

  if (!xsmp->connection) return;
...
static void sm_client_xsmp_startup(EggSMClient *client, const char *client_id) {
  ...
  xsmp->connection = SmcOpenConnection(
      NULL, xsmp, SmProtoMajor, SmProtoMinor,
      SmcSaveYourselfProcMask | SmcDieProcMask | SmcSaveCompleteProcMask |
          SmcShutdownCancelledProcMask,
      &callbacks, xsmp->client_id, &ret_client_id, sizeof(error_string_ret),
      error_string_ret);

  if (!xsmp->connection) {
    g_warning(
        "Failed to connect to the session manager: %s\n",
        error_string_ret[0] ? error_string_ret : "no error message given");
    xsmp->state = XSMP_STATE_CONNECTION_CLOSED;
    return;
  }
...
static void sm_client_xsmp_set_restart_command(EggSMClient *client, int argc,
  ...
  for (i = 0; i < argc; i++) xsmp->restart_command[i] = g_strdup(argv[i]);
...
static void sm_client_xsmp_set_discard_command(EggSMClient *client, int argc,
  ...
  for (i = 0; i < argc; i++) xsmp->discard_command[i] = g_strdup(argv[i]);
...
static void sm_client_xsmp_will_quit(EggSMClient *client, gboolean will_quit) {
  ...
  if (will_quit && xsmp->need_save_state) save_state(xsmp);
...
static gboolean sm_client_xsmp_end_session(EggSMClient *client,
  ...
  while (xsmp->state != XSMP_STATE_IDLE ||
    ...
    if (xsmp->shutting_down) return TRUE;
    ...
    switch (xsmp->state) {
      ...
      case XSMP_STATE_IDLE:
        ...
        if (!xsmp->expecting_initial_save_yourself) break;
        /* else fall through */
  ...
  g_debug(
      "Sending SaveYourselfRequest(SmSaveGlobal, Shutdown, SmInteractStyleAny, "
      "%sFast)",
...
static gboolean idle_do_pending_events(gpointer data) {
...
static void update_pending_events(EggSMClientXSMP *xsmp) {
  ...
  if (want_idle) {
    if (xsmp->idle == 0) xsmp->idle = g_idle_add(idle_do_pending_events, xsmp);
  } else {
    if (xsmp->idle != 0) g_source_remove(xsmp->idle);
    xsmp->idle = 0;
  }
}

static void fix_broken_state(EggSMClientXSMP *xsmp, const char *message,
  ...
  if (send_interact_done) SmcInteractDone(xsmp->connection, False);
  if (send_save_yourself_done) SmcSaveYourselfDone(xsmp->connection, True);

  xsmp->state =
      send_save_yourself_done ? XSMP_STATE_SAVE_YOURSELF_DONE : XSMP_STATE_IDLE;
}
...
static void xsmp_save_yourself(SmcConn smc_conn, SmPointer client_data,
  ...
  g_debug("Received SaveYourself(%s, %s, %s, %s) in state %s",
          ...
          : interact_style == SmInteractStyleErrors ? "SmInteractStyleErrors"
                                                    : "SmInteractStyleNone",
          fast ? "Fast" : "!Fast", EGG_SM_CLIENT_XSMP_STATE(xsmp));
...
static void save_state(EggSMClientXSMP *xsmp) {
  ...
  if (!state_file) {
    ...
    if (xsmp->set_discard_command) {
      ...
      set_properties(xsmp, ptrarray_prop(SmDiscardCommand, discard), NULL);
  ...
  if (desktop_file) {
    ...
    desktop_file_path = g_filename_from_uri(
        egg_desktop_file_get_source(desktop_file), NULL, NULL);
    if (desktop_file_path &&
        g_key_file_load_from_file(
            merged_file, desktop_file_path,
      ...
      for (g = 0; groups[g]; g++) {
        ...
        for (k = 0; keys[k]; k++) {
          value = g_key_file_get_value(state_file, groups[g], keys[k], NULL);
          if (value) {
            g_key_file_set_value(merged_file, groups[g], keys[k], value);
            g_free(value);
          }
        }
        g_strfreev(keys);
  ...
  while (1) {
    ...
    state_file_path = g_strdup_printf(
        "%s%csession-state%c%s-%ld.%s", g_get_user_config_dir(),
        G_DIR_SEPARATOR, G_DIR_SEPARATOR, g_get_prgname(),
        (long)time(NULL) + offset, desktop_file ? "desktop" : "state");
      ...
      if (errno == EEXIST) {
        ...
        *sep = '\0';
        if (g_mkdir_with_parents(state_file_path, 0755) != 0) {
          g_warning("Could not create directory '%s'", state_file_path);
          g_free(state_file_path);
  ...
  restart =
      generate_command(xsmp->restart_command, xsmp->client_id, state_file_path);
  set_properties(xsmp, ptrarray_prop(SmRestartCommand, restart), NULL);
  g_ptr_array_free(restart, TRUE);
  ...
  if (state_file_path) {
    set_properties(
        xsmp,
        array_prop(SmDiscardCommand, "/bin/rm", "-rf", state_file_path, NULL),
        NULL);
    g_free(state_file_path);
  }
}
...
static void xsmp_die(SmcConn smc_conn, SmPointer client_data) {
  ...
  g_debug("Received Die message in state %s", EGG_SM_CLIENT_XSMP_STATE(xsmp));
...
static GPtrArray *generate_command(char **restart_command,
  ...
  for (i = 1; restart_command[i]; i++) g_ptr_array_add(cmd, restart_command[i]);
...
static void set_properties(EggSMClientXSMP *xsmp, ...) {
  ...
  va_start(ap, xsmp);
  while ((prop = va_arg(ap, SmProp *))) g_ptr_array_add(props, prop);
  va_end(ap);

  if (xsmp->connection) {
    SmcSetProperties(xsmp->connection, props->len, (SmProp **)props->pdata);
  }
...
static void delete_properties(EggSMClientXSMP *xsmp, ...) {
  ...
  va_start(ap, xsmp);
  while ((prop = va_arg(ap, char *))) g_ptr_array_add(props, prop);
  va_end(ap);
...
static void ice_connection_watch(IceConn ice_conn, IcePointer client_data,
  ...
  if (opening) {
    ...
    watch_id = g_io_add_watch(channel, G_IO_IN | G_IO_ERR, ice_iochannel_watch,
                              ice_conn);
...
static void ice_io_error_handler(IceConn ice_conn) { /* Do nothing */ }
