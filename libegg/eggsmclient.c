...
static gboolean running_in_mate(void) {
...
EggSMClient *egg_sm_client_get(void) {
  if (!global_client) {
    if (!sm_client_disable) {
      ...
      if (!global_client) global_client = egg_sm_client_dbus_new();
#endif
    }
     ...
     */
    if (!global_client) global_client = g_object_new(EGG_TYPE_SM_CLIENT, NULL);
    /*FIXME
...
GKeyFile *egg_sm_client_get_state_file(EggSMClient *client) {
  ...
  if (!sm_client_state_file) return NULL;
  if (priv->state_file) return priv->state_file;
...
void egg_sm_client_quit_requested(EggSMClient *client) {
  ...
  if (!g_signal_has_handler_pending(client, signals[QUIT_REQUESTED], 0,
                                    FALSE)) {
